...
try:
    karx = SecureKarxController(access_token=os.getenv('KARX_ACCESS_TOKEN', 'default_token'))
...
def generate_code():
    try:
        ...
        if not result:
            logger.info("Falling back to KARX generation")
...
if __name__ == '__main__':
    ...
    if not os.getenv('KARX_ACCESS_TOKEN'):
        logger.warning("KARX access token not found in environment variables")
